...
function parseDateString(element) {
  return moment
    .tz(`${element.id} ${element.it}`, 'DD/MM/YYYY HH:mm', 'GMT')
    .tz('Europe/Amsterdam')
}
...
export default async () => {
  if (
    !lastExwaveDate ||
    moment().add(-refreshRate, 'minutes').unix() > lastExwaveParseCheck
  ) {
    await axios
      .get('https://www.p337.info/pokemongo/pages/ex-invites/', {
        responseType: 'text'
      })
      .then((response) => {
        ...
        console.log(
          `Refreshed the EX raid wave dates to: ${lastExwaveDate.format(
            'DD-MM-YYYY HH:mm'
          )} and ${secondToLastExwaveDate.format('DD-MM-YYYY HH:mm')}`
        )
      })
  }
